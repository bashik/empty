 ...
 * <ul>
 * <li>The first rule says that if the user requests the URL '/path/to/comment.php/articles',
 *   it should be treated as '/path/to/comment.php/article/list'; and vice versa applies
 ...
 *   '/path/to/comment.php/article/13', it should be treated as '/path/to/comment.php/article/read?id=13';
 ...
 * <pre>
 * array(
 *      '<_c:(post|comments)>/<id:\d+>/<_a:(create|update|delete)>'=>'<_c>/<_a>',
 *      '<_c:(post|comments)>/<id:\d+>'=>'<_c>/view',
 *      '<_c:(post|comments)>s/*'=>'<_c>/list',
 * )
 * </pre>
 * In the above, we use two named parameters '<_c>' and '<_a>' in the route part. The '<_c>'
 * parameter matches either 'post' or 'comments', while the '<_a>' parameter matches an action ID.
 *
 * Like normal rules, these rules can be used for both parsing and creating URLs.
 * For example, using the rules above, the URL '/comment.php/post/123/create'
 * would be parsed as the route 'post/create' with GET parameter 'id' being 123.
 * And given the route 'post/list' and GET parameter 'pages' being 2, we should get a URL
 * '/comment.php/posts/pages/2'.
 *
...
class CUrlManager extends CApplicationComponent
{
	 ...
	 * For example, ".html" can be used so that the URL looks like pointing to a static HTML pages. Defaults to empty.
	 */
	public $urlSuffix='';
	/**
...
class CUrlRule extends CBaseUrlRule
{
	 ...
	 * For example, ".html" can be used so that the URL looks like pointing to a static HTML pages.
