...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	public $authTimeout;
	/**
	 * @var boolean whether to automatically renew the identity cookie each time a pages is requested.
	 ...
	 * If that happens during an ajax call, the complete HTML login pages is returned as the result of that ajax call. That could be
	 * a problem if the ajax call expects the result to be a json array or a predefined string, as the login pages is ignored in that case.
	...
	/**
	 * Redirects the user browser to the login pages.
	 * Before the redirection, the current URL (if it's not an AJAX url) will be
	 * kept in {@link returnUrl} so that the user browser may be redirected back
	 * to the current pages after successful login. Make sure you set {@link loginUrl}
	 ...
	 * across pages requests during a user session.
	 *
	 * @param string $key variable name
