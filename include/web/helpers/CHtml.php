...
class CHtml
{
	 ...
	 * on the pages. Elements injected into the pages at a later time will not be processed.
	 *
	...
	/**
	 * Generates a meta tag that can be inserted in the head section of HTML pages.
	...
	/**
	 * Generates a link tag that can be inserted in the head section of HTML pages.
	 ...
	 * meta tag with {@link CClientScript} so that the pages can be refreshed in
	 * the specified seconds.
	 * @param integer $seconds the number of seconds to wait before refreshing the pages
	 * @param string $url the URL to which the pages should be redirected to. If empty, it means the current pages.
	 * @since 1.1.1
	 */
	 ...
	 * hidden field for storing persistent pages states. You should use this method to generate
	 * a form tag if you want to access persistent pages states when the form is submitted.
	 * @param mixed $action the form action URL (see {@link normalizeUrl} for details about this parameter.)
	 * @param string $method form method (e.g. post, get)
	 * @param array $htmlOptions additional HTML attributes (see {@link tag}).
	 * @return string the generated form tag.
	...
	/**
	 * Generates a hidden field for storing persistent pages states.
	 * This method is internally used by {@link statefulForm}.
	 * @param string $value the persistent pages states in serialized format
...
EOD;
	 ...
	 * For example, <code>array('post/list', 'pages'=>3)</code> may be used to generate the URL
	 * <code>/comment.php?r=post/list&pages=3</code>.
	 *
